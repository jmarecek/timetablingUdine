 ...
 */
boolean graph_write_dimacs_ascii_file(graph_t *g, const char *comment, const char *file) {
	FILE *fp;
 ...
 * Returns TRUE if successful, FALSE if an error occurred.
 */
boolean graph_write_dimacs_ascii(graph_t *g, const char *comment, FILE *fp) {
	int i,j;
 ...
 * Returns TRUE if successful, FALSE if an error occurred.
 */
boolean graph_write_dimacs_binary_file(graph_t *g, const char *comment, const char *file) {
	FILE *fp;
...
boolean graph_write_dimacs_binary(graph_t *g, const char *comment,FILE *fp) {
	char *buf;
	char *header=NULL;
	int headersize=0;
	int headerlength=0;
	...
	buf=(char*)malloc(MAX(1024,g->n/8+1));
...
static graph_t *graph_read_dimacs_binary(FILE *fp,char *firstline) {
	...
	/* Binary part. */
	buf=(char**)calloc(g->n,sizeof(char*));
	for (i=0; i < g->n; i++) {
		buf[i]=(char*)calloc(g->n,1);
